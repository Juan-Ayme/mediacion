...
const SectionHeader = ({ title, subtitle, icon: Icon, dark = false }) => (
  <div className="text-center mb-12 md:mb-16 relative z-10 px-4">
    ...
    <h2 className={`text-3xl md:text-5xl lg:text-6xl font-black tracking-tight mb-4 md:mb-6 ${dark ? 'text-white' : 'text-slate-900'}`}>
      {title}
    </h2>
    <div className="relative h-1.5 w-16 md:w-24 mx-auto bg-slate-200/50 rounded-full overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-500 to-teal-400 w-1/2 animate-shimmer"></div>
    </div>
  </div>
);
    ...
    <nav className={`fixed w-full z-50 transition-all duration-500 ${scrolled ? 'bg-white/90 backdrop-blur-md border-b border-slate-200/50 py-3' : 'bg-transparent py-4 md:py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 md:px-6 flex justify-between items-center">
        <div className={`flex items-center gap-2 font-black text-lg md:text-xl tracking-tighter transition-colors ${scrolled ? 'text-slate-900' : 'text-white'}`}>
          ...
          <span>MEDIACIÓN<span className="text-emerald-500">.UNSCH</span></span>
        </div>
        ...
        <div className="hidden md:flex gap-1">
          {links.map(link => (
            <a 
              key={link.name} 
              href={link.href} 
              className={`px-4 lg:px-5 py-2 rounded-full text-xs lg:text-sm font-bold transition-all ${scrolled ? 'text-slate-600 hover:bg-slate-100 hover:text-emerald-600' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}
            >
              {link.name}
            </a>
          ))}
...
const Hero = () => (
  <div className="relative min-h-screen flex items-center justify-center bg-slate-900 overflow-hidden pt-20 px-4 md:px-6">
    <div className="absolute inset-0">
        <div className="absolute top-[-20%] left-[-10%] w-[300px] md:w-[800px] h-[300px] md:h-[800px] bg-emerald-500/20 rounded-full blur-[80px] md:blur-[120px] mix-blend-screen animate-pulse-slow"></div>
        <div className="absolute bottom-[-20%] right-[-10%] w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-blue-600/20 rounded-full blur-[80px] md:blur-[100px] mix-blend-screen animate-pulse-slow delay-75"></div>
    ...
    <div className="relative z-10 max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
      <div className="text-center lg:text-left space-y-6 md:space-y-8 animate-fade-in-up">
        <div className="inline-flex items-center gap-3 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm text-emerald-300 text-[10px] md:text-xs font-bold tracking-widest uppercase">
        ...
        <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-black text-white leading-[1.1] lg:leading-[0.95] tracking-tight">
          Paz Social y<br/>
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-300 to-blue-400">Resolución</span>
        </h1>
        
        <p className="text-base md:text-xl text-slate-400 max-w-lg mx-auto lg:mx-0 leading-relaxed font-light">
          Un enfoque humano y estratégico para transformar conflictos en oportunidades. Teoría, práctica y normativa.
        </p>

        <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-4">
          <a href="#historia" className="px-6 md:px-8 py-3 md:py-4 bg-emerald-500 hover:bg-emerald-400 text-slate-900 font-black rounded-2xl shadow-lg shadow-emerald-500/20 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-2 text-sm md:text-base">
            Iniciar Recorrido <ArrowRight className="w-4 h-4 md:w-5 md:h-5" />
          </a>
          <div className="px-6 md:px-6 py-3 md:py-4 bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 text-slate-300 font-medium flex items-center justify-center gap-2 text-sm md:text-base">
      ...
      <div className="relative hidden lg:block perspective-1000">
        <div className="relative z-10 bg-gradient-to-br from-slate-800 to-slate-900 p-3 rounded-[2.5rem] shadow-2xl border border-slate-700 transform rotate-y-12 rotate-x-6 hover:rotate-y-0 hover:rotate-x-0 transition-all duration-700 group">
          <div className="absolute inset-0 bg-emerald-500/20 blur-2xl -z-10 rounded-[2.5rem]"></div>
          <img 
          ...
          <div className="absolute -bottom-10 -left-10 bg-white p-6 rounded-2xl shadow-xl max-w-xs animate-float border border-slate-100 hidden xl:block">
...
const TimelineSection = () => {
  ...
  return (
    <section className="py-16 md:py-24 px-4 md:px-6 bg-slate-50 relative overflow-hidden" id="historia">
      <div className="absolute top-0 right-0 w-96 h-96 bg-emerald-100/50 rounded-full blur-3xl translate-x-1/2 -translate-y-1/2 pointer-events-none"></div>
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex justify-center mb-12 md:mb-16">
            <div className="bg-white p-1.5 md:p-2 rounded-full shadow-lg border border-slate-100 inline-flex relative w-full max-w-xs md:max-w-md">
                <div 
                    className={`absolute top-1.5 bottom-1.5 md:top-2 md:bottom-2 w-[calc(50%-6px)] md:w-[calc(50%-8px)] bg-slate-900 rounded-full transition-all duration-500 ease-spring ${activeTab === 'ancient' ? 'left-1.5 md:left-2' : 'left-[calc(50%+3px)] md:left-[calc(50%+4px)]'}`}
                ></div>
                <button 
                    onClick={() => setActiveTab('ancient')}
                    className={`flex-1 relative z-10 px-4 py-2.5 md:px-8 md:py-3 rounded-full text-xs md:text-sm font-bold tracking-wide transition-colors duration-300 ${activeTab === 'ancient' ? 'text-white' : 'text-slate-500 hover:text-slate-900'}`}
                >
                    Edad Antigua
                </button>
                <button 
                    onClick={() => setActiveTab('modern')}
                    className={`flex-1 relative z-10 px-4 py-2.5 md:px-8 md:py-3 rounded-full text-xs md:text-sm font-bold tracking-wide transition-colors duration-300 ${activeTab === 'modern' ? 'text-white' : 'text-slate-500 hover:text-slate-900'}`}
                >
                    Edad Moderna
                </button>
            </div>
        ...
        <div className="relative min-h-[500px]">
            <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-0.5 bg-gradient-to-b from-emerald-200 via-emerald-400 to-emerald-200 hidden md:block"></div>
                ...
                {historyData[activeTab].map((item, idx) => (
                    <div key={idx} className={`flex items-center justify-between flex-col md:flex-row group ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''} animate-fade-in-up`} style={{animationDelay: `${idx * 100}ms`}}>
                        ...
                        <div className="w-full md:w-5/12 mb-4 md:mb-0">
                            <div className={`p-6 md:p-8 bg-white rounded-2xl md:rounded-3xl shadow-lg shadow-slate-200/50 border border-slate-100 hover:border-emerald-200 transition-all duration-300 transform hover:-translate-y-1 ${idx % 2 === 0 ? 'text-left' : 'md:text-right text-left'}`}>
                                <span className="inline-block px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-xs md:text-sm font-black mb-3">{item.year}</span>
                                <h3 className="text-lg md:text-xl font-bold text-slate-900 mb-2">{item.title}</h3>
                                <p className="text-slate-600 leading-relaxed text-sm">{item.desc}</p>
                            </div>
                        </div>
                        
                        <div className="z-10 bg-white w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center border-4 border-emerald-100 shadow-lg order-first md:order-none mb-4 md:mb-0 group-hover:scale-110 transition-transform duration-300">
...
const SchoolsSection = () => (
  <section className="py-16 md:py-24 px-4 md:px-6 bg-slate-900 text-white" id="escuelas">
    <div className="max-w-7xl mx-auto">
      <SectionHeader title="Escuelas y Modelos" subtitle="Fundamentos Teóricos" icon={BookOpen} dark />
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {schools.map((school, idx) => (
          <div key={idx} className="group relative bg-slate-800 rounded-[2rem] p-6 md:p-8 border border-slate-700/50 hover:bg-slate-800/80 transition-all duration-500 hover:-translate-y-2 overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col h-full">
              <div className="flex justify-between items-start mb-6">
                <div className="inline-block px-3 py-1 bg-slate-900/50 backdrop-blur-md rounded-lg border border-slate-600 text-emerald-400 text-[10px] md:text-xs font-bold uppercase tracking-wider">
                  {school.focus}
                </div>
                <Star className="w-5 h-5 md:w-6 md:h-6 text-slate-600 group-hover:text-yellow-400 transition-colors fill-current" />
              </div>
              
              <h3 className="text-xl md:text-2xl font-bold mb-2 text-white group-hover:text-emerald-300 transition-colors">{school.name}</h3>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wide mb-4 md:mb-6">Autor: {school.author}</p>
              
              <div className="mt-auto">
                <p className="text-slate-300 leading-relaxed text-sm border-l-2 border-emerald-500/30 pl-4 group-hover:border-emerald-500 transition-colors">
                    {school.desc}
                </p>
              </div>
            </div>
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="space-y-6">
        {stages.map((stage, idx) => (
          <div key={idx} className="group relative bg-white rounded-2xl p-6 md:p-8 shadow-lg shadow-slate-200/50 border border-slate-100 hover:border-emerald-100 transition-all duration-300 overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start md:items-center relative z-10">
              ...
              <div className="hidden md:flex items-center justify-center w-12 h-12 rounded-full border border-slate-100 text-slate-300 group-hover:bg-emerald-500 group-hover:text-white group-hover:border-emerald-500 transition-all duration-300">
...
const RoleSection = () => (
  <section className="py-16 md:py-24 px-4 md:px-6 bg-slate-50" id="mediador">
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid lg:grid-cols-12 gap-8 lg:gap-16">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-xl shadow-slate-200/40 border border-slate-100 sticky top-24">
            <h3 className="text-xl md:text-2xl font-black text-slate-900 mb-6 flex items-center gap-2">
                <Anchor className="w-5 h-5 md:w-6 md:h-6 text-emerald-500"/>
                Perfil del Mediador
            </h3>
            <div className="space-y-6">
                {mediatorSkills.map((skill, idx) => (
                <div key={idx} className="flex gap-4 items-start group">
                    <div className="p-2.5 md:p-3 bg-slate-50 rounded-2xl text-emerald-600 group-hover:bg-emerald-500 group-hover:text-white transition-all duration-300 shadow-sm shrink-0">
                    {skill.icon}
                    </div>
                    <div>
                    <h4 className="font-bold text-slate-900 text-base md:text-lg">{skill.title}</h4>
                    <p className="text-xs md:text-sm text-slate-500 mt-1 leading-relaxed">{skill.desc}</p>
                    </div>
                </div>
                ))}
            </div>
        ...
        <div className="lg:col-span-8">
           <div className="bg-slate-900 text-white rounded-[2.5rem] p-6 md:p-12 shadow-2xl relative overflow-hidden">
             <div className="absolute top-0 right-0 w-[200px] md:w-[400px] h-[200px] md:h-[400px] bg-emerald-500/20 rounded-full blur-[100px] translate-x-1/2 -translate-y-1/2"></div>
             
             <div className="relative z-10">
                <h3 className="text-2xl md:text-3xl font-black mb-8 flex items-center gap-3">
                    <Mic className="w-6 h-6 md:w-8 md:h-8 text-emerald-400" /> 
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <ul className="space-y-4">
                        ...
                        <li key={i} className="flex items-center gap-3 text-slate-300 font-medium bg-red-500/5 p-3 rounded-xl border border-red-500/10 text-xs md:text-sm">
                    ...
                    {/* Do's */}
                    <div className="bg-emerald-500/10 backdrop-blur-md p-6 md:p-8 rounded-3xl border border-emerald-500/20 hover:bg-emerald-500/20 transition-colors">
                    <div className="flex items-center gap-3 mb-6 text-emerald-400">
                        <div className="p-2 bg-emerald-500/20 rounded-lg"><CheckCircle className="w-4 h-4 md:w-5 md:h-5"/></div>
                        <span className="font-bold uppercase tracking-wider text-xs md:text-sm">Lo que CONSTRUYE</span>
                    </div>
                    <ul className="space-y-4">
                        {phrases.positive.map((p, i) => (
                        <li key={i} className="flex items-center gap-3 text-white font-medium bg-emerald-500/10 p-3 rounded-xl border border-emerald-500/20 shadow-lg text-xs md:text-sm">
  ...
  <section className="py-16 md:py-24 px-4 md:px-6 bg-white" id="comparativo">
    <div className="max-w-6xl mx-auto">
      ...
      <div className="relative grid md:grid-cols-2 gap-6 items-stretch">
        {/* VS Badge */}
        <div className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 hidden md:flex w-16 h-16 md:w-20 md:h-20 bg-white rounded-full items-center justify-center border-4 border-slate-100 text-xl md:text-2xl font-black text-slate-900 shadow-xl">
          VS
        </div>

        {/* Mediation Card */}
        <div className="bg-emerald-50 p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-emerald-100 hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-center gap-4 mb-6 md:mb-8">
             <div className="w-12 h-12 md:w-14 md:h-14 bg-emerald-500 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-emerald-500/30 group-hover:scale-110 transition-transform shrink-0">
                <HeartHandshake className="w-6 h-6 md:w-8 md:h-8"/>
             </div>
             <div>
                 <span className="text-emerald-600 font-bold tracking-wider text-[10px] md:text-xs uppercase">Autocomposición Pura</span>
                 <h3 className="text-2xl md:text-3xl font-black text-slate-900">MEDIACIÓN</h3>
             </div>
          </div>
          <ul className="space-y-6">
            <li className="flex gap-4 items-start">
              <div className="w-6 h-6 rounded-full bg-emerald-200 flex items-center justify-center font-bold text-xs text-emerald-800 mt-1 shrink-0">1</div>
              <div>
                  <strong className="text-slate-900 block text-sm uppercase mb-1">Rol del Tercero</strong>
                  <p className="text-slate-600 text-sm">Facilitador pasivo. <span className="font-bold text-emerald-600 bg-emerald-100 px-1 rounded">NO propone</span> soluciones.</p>
              </div>
            </li>
            <li className="flex gap-4 items-start">
              <div className="w-6 h-6 rounded-full bg-emerald-200 flex items-center justify-center font-bold text-xs text-emerald-800 mt-1 shrink-0">2</div>
              <div>
                  <strong className="text-slate-900 block text-sm uppercase mb-1">Poder</strong>
                  <p className="text-slate-600 text-sm">Las partes tienen control total.</p>
              </div>
            </li>
          </ul>
        </div>
        ...
        {/* Conciliation Card */}
        <div className="bg-slate-50 p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-slate-200 hover:shadow-xl transition-all duration-300 group">
          <div className="flex items-center gap-4 mb-6 md:mb-8">
             <div className="w-12 h-12 md:w-14 md:h-14 bg-slate-800 rounded-2xl flex items-center justify-center text-white shadow-lg group-hover:scale-110 transition-transform shrink-0">
                <Gavel className="w-6 h-6 md:w-8 md:h-8"/>
             </div>
             <div>
                 <span className="text-slate-500 font-bold tracking-wider text-[10px] md:text-xs uppercase">Intervención Activa</span>
                 <h3 className="text-2xl md:text-3xl font-black text-slate-900">CONCILIACIÓN</h3>
             </div>
          </div>
          <ul className="space-y-6">
            <li className="flex gap-4 items-start">
              <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center font-bold text-xs text-slate-600 mt-1 shrink-0">1</div>
              <div>
                  <strong className="text-slate-900 block text-sm uppercase mb-1">Rol del Tercero</strong>
                  <p className="text-slate-600 text-sm">Activo. <span className="font-bold text-slate-900 bg-slate-200 px-1 rounded">SÍ propone</span> fórmulas.</p>
              </div>
            </li>
            <li className="flex gap-4 items-start">
              <div className="w-6 h-6 rounded-full bg-slate-200 flex items-center justify-center font-bold text-xs text-slate-600 mt-1 shrink-0">2</div>
              <div>
                  <strong className="text-slate-900 block text-sm uppercase mb-1">Poder</strong>
                  <p className="text-slate-600 text-sm">El conciliador influye en el resultado final.</p>
              </div>
            </li>
          </ul>
        </div>
      ...
      <div className="mt-16 md:mt-20">
        <h4 className="text-center text-slate-400 uppercase tracking-[0.2em] text-xs font-bold mb-8">Perspectiva Internacional</h4>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {comparativeLaw.map((item, idx) => (
            <div key={idx} className="bg-white p-6 rounded-2xl border border-slate-100 text-center hover:border-emerald-200 hover:shadow-lg transition-all">
              <div className="text-3xl md:text-4xl mb-3 transform hover:scale-110 transition-transform duration-300">{item.flag}</div>
              <div className="font-bold text-slate-900 text-sm mb-2">{item.country}</div>
          ...
          <div key={idx} className="group relative bg-white rounded-3xl overflow-hidden shadow-xl shadow-slate-200/50 border border-slate-100 hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
            ...
            <div className="p-6">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h3 className="font-bold text-slate-900 text-base md:text-lg leading-tight">{video.title}</h3>
                </div>
                <p className="text-slate-500 text-xs md:text-sm ml-11 md:ml-13 border-l-2 border-emerald-100 pl-3">{video.desc}</p>
            </div>
          </div>
        ))}
      </div>
...
const TeamSection = () => {
  ...
  return (
    <section className="py-16 md:py-24 px-4 md:px-6 bg-slate-900 text-white overflow-hidden" id="equipo">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8 md:gap-12 items-center">
          ...
          <div className="w-full lg:w-1/2 space-y-2 md:space-y-3 h-[300px] md:h-[450px] overflow-y-auto pr-2 md:pr-4 custom-scrollbar">
            {teamMembers.map((member, idx) => (
              ...
              >
                <span className={`font-bold text-xs md:text-base leading-tight ${activeMember === idx ? 'text-white' : 'text-slate-400 group-hover:text-white'}`}>
                  {member}
                </span>
                {activeMember === idx && <ChevronRight className="w-4 h-4 md:w-5 md:h-5 text-emerald-200 animate-pulse shrink-0" />}
              </button>
            ))}
          </div>
          ...
          <div className="w-full lg:w-1/2 flex justify-center">
            <div className="relative bg-gradient-to-br from-slate-800 to-slate-900 p-2 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl w-full max-w-sm transform transition-all duration-500 hover:scale-105 border border-slate-700">
              <div className="absolute inset-0 bg-emerald-500/20 blur-3xl -z-10 rounded-full"></div>
              <div className="bg-slate-950 rounded-[1.8rem] md:rounded-[2.3rem] p-8 md:p-10 text-center h-full flex flex-col items-center justify-center min-h-[350px] md:min-h-[420px] relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-full bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10"></div>
                
                <div className="relative z-10 w-24 h-24 md:w-32 md:h-32 bg-gradient-to-br from-emerald-400 to-teal-600 rounded-full flex items-center justify-center text-4xl md:text-6xl font-black text-white mb-6 md:mb-8 shadow-xl shadow-emerald-500/30">
                ...
                <p className="relative z-10 text-emerald-400 font-bold uppercase tracking-widest text-[10px] md:text-xs mb-6 md:mb-8 bg-emerald-900/30 px-3 py-1 rounded-full border border-emerald-500/30">
                  Futuro Abogado
                </p>
...
const App = () => {
  return (
    <div className="font-sans antialiased text-slate-600 selection:bg-emerald-500 selection:text-white bg-white">
      <Navbar />
      <Hero />
      <TimelineSection />
      <SchoolsSection />
      ...
      <section className="py-16 md:py-24 px-4 md:px-6 bg-emerald-600 text-white text-center relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <p className="text-lg md:text-xl lg:text-2xl font-light mb-8 md:mb-12 text-emerald-50 max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-emerald-900">
            {[
                ...
                <div key={i} className="bg-white p-6 md:p-8 rounded-3xl shadow-xl transform hover:-translate-y-2 transition-all duration-300">
                  ...
                  <p className="text-xs md:text-sm font-medium text-emerald-800/70">{card.desc}</p>
                </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8">
          <div className="text-center md:text-left">
            <h4 className="text-white font-bold text-base md:text-lg mb-2 flex items-center gap-2 justify-center md:justify-start">
              <Scale className="w-4 h-4 md:w-5 md:h-5 text-emerald-500"/> UNSCH
            </h4>
            <p className="text-xs md:text-sm text-slate-400">Teoría del Conflicto y Mecanismos de Resolución</p>
          </div>
          <div className="text-center md:text-right">
             <p className="text-[10px] md:text-xs text-slate-600 mb-2">Docente: Paola Capcha Cabrera</p>
             <p className="text-[10px] md:text-xs text-slate-600">&copy; 2025 Grupo 03. Ayacucho, Perú.</p>
          </div>
        </div>
      </footer>
